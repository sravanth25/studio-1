          ...
          <div className="flex flex-col gap-4 items-center md:items-start">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="gap-4" variant="outline" asChild>
                <a href="https://www.jaaga.ai/telangana" target="_blank" rel="noopener noreferrer">
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetchBlogs().then(posts => {
      const sorted = [...posts].sort((a, b) => (b.id || 0) - (a.id || 0)).slice(0, 3);
      setFeaturedPosts(sorted);
    });
  }, []);

  const faqsLeft = [
    { question: "What are property documents?", answer: "Property documents are official records that prove ownership, land details, boundaries, and transaction history of a property." },
    { question: "Which documents are required to verify property ownership?", answer: "Key documents include the Sale Deed, Encumbrance Certificate (EC), Khata or Patta, and relevant land records depending on the state." },
    { question: "What is an Encumbrance Certificate (EC)?", answer: "An Encumbrance Certificate shows all registered transactions related to a property and helps verify whether it has any legal or financial liabilities." },
    { question: "What is a Sale Deed?", answer: "A Sale Deed is a legally registered document that confirms the transfer of property ownership from seller to buyer." },
    { question: "What is Khata or Patta?", answer: "Khata and Patta are property records maintained by local authorities that contain ownership and property tax details." },
  ];

  const faqsRight = [
    { question: "What are Adangal, ROR, and 1B documents?", answer: "These are government land records that show ownership, survey numbers, land extent, land type, and usage, mainly for agricultural land." },
    { question: "What is an FMB Sketch?", answer: "An FMB Sketch is a government land map showing exact property boundaries and measurements based on survey records." },
    { question: "What is Bhu-Aadhaar (ULPIN)?", answer: "Bhu-Aadhaar or ULPIN is a unique digital identification number assigned to each land parcel, linking land records digitally." },
    { question: "Are online property documents legally valid?", answer: "Online documents are valid for verification purposes, but certified copies are recommended for legal or official use." },
    { question: "Why is property document verification important?", answer: "Verification helps prevent fraud, ownership disputes, encroachments, and legal issues during property transactions." },
  ];
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category) => {
              ...
              const href = isExternalLink ? externalLinks[category.slug] : `/category/${category.slug}`;

              const CardWrapper = isExternalLink ? (
                <a key={category.slug} href={href} className="group" target="_blank" rel="noopener noreferrer">
        ...
        <div className="container mx-auto px-4">
          <div className="text-center md:text-left md:flex justify-between items-center mb-12">
            <div className="space-y-2">
              ...
              <p className="text-lg text-muted-foreground">Stay updated with the latest trends and advice from our experts.</p>
            </div>
      ...
      <section id="cta" className="py-16 md:py-24 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="bg-primary text-primary-foreground rounded-xl p-8 md:p-12 text-center">
            <h2 className="font-headline text-3xl md:text-4xl font-bold mb-4">Ready to Secure Your Property?</h2>
            <p className="text-lg max-w-2xl mx-auto mb-8">
              Don't leave your property to chance. Our digital solutions provide the clarity and security you need.
            </p>
            <Button asChild size="lg" variant="secondary" className="font-bold bg-background text-primary hover:bg-background/90">
